...
async function activateSerialLicense(unityPath, username, password, serial) {
    ...
    const stdout = await executeUnity(unityPath, `-batchmode -nographics -quit -projectPath "chatlyst-dev" -executeMethod SyncVS_Workaround.SyncSolution -username "${username}" -password "${password}" -serial "${serial}"`);
